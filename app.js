...
const getInputValue = (event) => {
    ...
    featchAllSuperHero(searchText);
}

// search for submission
searchForm.addEventListener('submit', getInputValue);

// api key => 1375620380035065
const featchAllSuperHero = async (searchText) => {
    let url = `https://www.superheroapi.com/api.php/1375620380035065/search/${searchText}`;
...
searchForm.search.addEventListener('keyup', () => {
    if(searchForm.search.value.length > 1){
        featchAllSuperHero(searchForm.search.value);
    }else{
        searchList.innerHTML = "";
    }
});
